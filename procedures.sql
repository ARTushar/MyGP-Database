...
CREATE OR REPLACE PROCEDURE PURCHASE_SMS_OFFER(IN OFFER_NO NUMERIC, IN USER_NO NUMERIC) AS $$
  ...
  BEGIN
    ...
    IF OLD_ACCOUNT_BALANCE >= COST THEN
      END_DATE := TO_CHAR(PURCHASED_TIMESTAMP + '1 DAY' * VALID, 'YYYY-MM-DD HH12:MI:SS AM');
    ...
    ELSE
      RAISE EXCEPTION USING ERRCODE = 'NO', MESSAGE = 'HAVE NOT SUFFICIENT BALANCE';
    END IF;

    EXCEPTION
    WHEN SQLSTATE 'NO' THEN
...
CREATE OR REPLACE FUNCTION COUNT_GENERAL_OFFER_PRICE(IN SMS, IN DATA, IN TALK)
 RETURNS NUMERIC AS $$
DECLARE
  PRICE NUMERIC;
  SMS_COST NUMERIC;
